...
def venues():
  ...
  for area in venues_data:
    state_city = f'${area.city} {area.state}'
    ...
    if dataHash.get(state_city, None) != None:
      dataHash[state_city]["venues"].append(
        {
          "id": area.id,
          "name": area.name,
          "num_upcoming_shows": len(db.session.query(Show).filter(Show.venue_id==area.id).filter(Show.start_time>datetime.now()).all())
        } 
      )
    else:
      my_object["venues"] = [
      ...
      dataHash[state_city] = my_object

  for key in dataHash:
    data.append(dataHash[key])
